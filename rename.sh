...
-o, --old-package-name [OLD_PACKAGE]     old package  name to be changed (i.e. com.example.projectname)
EOF
...
renamefolderstructure() {
  ...
  # mv $DIR/oldpackage $DIR/
  # rmdir $DIR/com/example
  # rmdir $DIR/com
...
LC_ALL=C find $WORKING_DIR/android -type f -exec sed -i "" "s/$OLD_PACKAGE_NAME_ESCAPED/$PACKAGE_NAME_ESCAPED/g" {} +
LC_ALL=C find $WORKING_DIR/android -type f -exec sed -i "" "s/$OLD_TITLE/$TITLE_NO_SPACES/g" {} +
